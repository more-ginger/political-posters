...
;
 ...
 = result.html.children.map((v) => {
		...
		for (const key of ['instance', 'module', 'css'] ) {
...
function node_contains_props_rune(node) {
...
function create_generated_name(script, base) {
	let name = base;
	let i = 1;
...
function create_props_rune_conflict_error(filename) {
	return new Error(
...
function create_layout_props_name(script) {
	return create_generated_name(script || '', layout_props_name);
}

function get_line_indentation(source, index) {
...
function get_layout_prop_name(property) {
		...
		visit$6(tree, 'root', (node) => {
			...
			const runes_layout_forwarding =
				import_script && use_runes_layout_props && instance[0]
					? get_props_rune_layout_forwarding(instance[0].value )
					: undefined;
			const layout_props = create_layout_props_name(
				instance[0] && (instance[0].value )
			);
...
const code_highlight = async (
	...
	//@ts-ignore
	if (!Prism) Prism = await import('./prism-Bz7tCQfu.js').then(function (n) { return n.p; });
...
//# sourceMappingURL=main-Bo3idoP1.js.map
